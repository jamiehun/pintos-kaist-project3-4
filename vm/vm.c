...
static struct frame *vm_get_frame (void) {
	...
	/* 3) 페이지 할당이 실패했을 경우 PANIC("to do")로 해당케이스들 표시 */
	if (frame->kva == NULL)
		PANIC("TO DO");
...
bool vm_claim_page (void *va) {
	...
	// ㄴ 의문 해결! : va와 spt에 올려주는 행위는 그 전에 끝내있었음 (vm_alloc_page_with_initializer)
